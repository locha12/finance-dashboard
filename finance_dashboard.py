...
# ================== OVERVIEW TAB (pie chart now shows $ + %) ==================
with tab1:
    ...
    st.subheader("Major Expense Categories (like your Excel)")
    ...
    summary_df = pd.DataFrame({
        ...
        "%": [round(x / total_exp * 100 if total_exp > 0 else 0, 1) for x in [housing, food, supp_total, health]]
    })
    st.dataframe(summary_df.style.format({"Amount": "${:,.0f}", "%": "{:.1f}%"}), use_container_width=True, hide_index=True)
    ...
    # 🔥 UPDATED PIE CHART — now shows $ amount + % on every slice
    ...
    total = alloc_data["Value"].sum()
    alloc_data["Display"] = alloc_data.apply(
        lambda x: f"${x['Value']:,.0f}<br>({x['Value']/total*100:.1f}%)" if total > 0 else "$0", axis=1
    )

    fig_alloc = px.pie(alloc_data, names="Category", values="Value", title="Investment Allocation (Live Values)")
    fig_alloc.update_traces(
        text=alloc_data["Display"],
        textinfo="text",
        textposition="inside",
        textfont_size=12,
        hovertemplate="<b>%{label}</b><br>$%{value:,.0f}<br>%{percent:.1f}%<extra></extra>"
    )
    st.plotly_chart(fig_alloc, use_container_width=True)

# ================== EXPENSES & INVESTMENTS TABS (unchanged) ==================
with tab2:
    ...
    st.subheader("Supplements Breakdown (detailed list — total feeds into Supplements)")
...
st.caption("All data saved locally.")
